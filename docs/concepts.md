...
cleanResearch works because it gives your AI a clean desk, clear instructions, and a project notebook. It does not make scientific decisions for you. It helps the AI keep the files and evidence ready for your judgment.
...
A messy project makes this worse. If five scripts appear to do the same job, the AI must guess which one is current. If a result has no link to its data or settings, the AI may find the number but not its meaning. cleanResearch gives each kind of information one fixed place, so the AI can find the right item with less guesswork.
...
[`AGENTS.md`](../AGENTS.md) contains the shared cleanResearch contract. It tells the AI where files belong, how to record an experiment, how to preserve the exact code that ran, and when it must ask you before changing scientific content. Agents that follow the `AGENTS.md` standard can use these rules.
...
cleanResearch does not ship any hooks today. The current skills and `AGENTS.md` tell the AI what to do, but they do not technically block an incorrect action. This is why the audit workflow matters.
